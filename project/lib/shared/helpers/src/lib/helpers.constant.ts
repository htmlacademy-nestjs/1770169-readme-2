export const REGEX = /%([^%]*)%/;
export const TIME_REGEX = /^(\d+)([shdmy])/;
export const USER_URL = 'http://localhost:5000/api/user';
export const APP_DEFAULT_PORT = 5000;
export const APP_VALIDATE_ERROR_MESSAGE = '[App Config Validation Error]: %message%.';
export const DEFAULT_MONGO_PORT = 27017;
export const MONGO_VALIDATE_ERROR_MESSAGE = '[Mongo Config Validation Error]: %message%.';
export const RABBITMQ_DEFAULT_PORT = 8026;
export const RABBITMQ_VALIDATE_ERROR_MESSAGE = '[RabbitMQ Config Validation Error]: %message%.';
export const TYPE_EXCHANGE = 'direct';
export const WRONG_TIME_ERROR = '[parseTime] Wrong time string: %time%';
export const VALUE_PARSE_ERROR = `[parseTime] Can't parse value count. Result is NaN.`;


